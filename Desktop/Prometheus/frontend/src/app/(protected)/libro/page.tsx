...
export default function LibroPage({ user: initialUser }: LibroPageProps = {}) {
  ...
  const handleDragEnd = async (result: DropResult, provided: ResponderProvided) => {
    ...
    // Prepare data for Supabase update - include all necessary fields
    const updates = items.map((item, index) => ({
      ...
      seme_id: item.seme_id || null,
      icona: item.icona,
      raw_interaction_session_id: item.raw_interaction_session_id,
      ordine: index,
                    ...
                    {chapters.map((chapter, index) => (
                      <Draggable key={chapter.id.toString()} draggableId={chapter.id.toString()} index={index}>
                        {(provided, snapshot) => (
                          ...
                          >
                            {/* CONTENUTO MIGLIORATO SENZA ID E ORDINE */}
                            <div className="flex-grow">
                              <div className="flex items-center">
                                <GripVertical className="h-5 w-5 text-muted-foreground mr-3 flex-shrink-0" />
                                <div className="flex-grow">
                                  <h3 className="font-medium text-lg text-gray-900 mb-1">{chapter.titolo}</h3>
                                  {chapter.sottotitolo && (
                                    <p className="text-sm text-gray-600 mb-2">{chapter.sottotitolo}</p>
                                  )}
                                  <p className="text-sm text-gray-500 line-clamp-2">
                                    {chapter.testo.substring(0, 150)}{chapter.testo.length > 150 ? '...' : ''}
                                  </p>
                                </div>
                              </div>
                            </div>
